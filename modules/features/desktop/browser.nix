let
  hmModule = { config, pkgs, ... }: let
    defaultOptions = {
      extensions = { # nix flake show "gitlab:rycee/nur-expressions?dir=pkgs/firefox-addons"
        force = true;
  ...
  in {
    programs.librewolf = {
      enable = true;
      profiles."${config.my.user.name}" = defaultOptions // {
        id = 0;
        isDefault = true;
      };
      profiles.work = defaultOptions // {
        id = 1;
        isDefault = false;
        extensions = { # nix flake show "gitlab:rycee/nur-expressions?dir=pkgs/firefox-addons"
          force = true;
